...
const USE_FIREBASE = false; // ← flip to true when you have Firebase config

const FIREBASE_CONFIG = {
  // Paste your Firebase config here when ready:
  // apiKey: "AIza...",
  // authDomain: "record-chief.firebaseapp.com",
  // projectId: "record-chief",
  // storageBucket: "record-chief.appspot.com",
  // messagingSenderId: "...",
  // appId: "..."
};

// ── Auth API ── used by SignupScreen, LoginScreen, App
// All screens call ONLY these functions — never localStorage directly
const AuthAPI = {

  // Sign up with email + password
  async signUp({ name, email, phone, location, password, sectors }) {
    if (USE_FIREBASE) {
      // Firebase path (active when USE_FIREBASE = true)
      ...
      const fbUser = { uid: cred.user.uid, name, email: email.toLowerCase(), phone, location, sectors, createdAt: new Date().toISOString(), emailVerified: false, authProvider: "firebase" };
      // Still save profile data to localStorage (or backend later)
      localStorage.setItem(`sl_profile_${cred.user.uid}`, JSON.stringify(fbUser));
      return { ok: true, user: fbUser, message: "Account created! Check your email to verify." };
    } else {
      // localStorage path (current)
      const existing = AuthAPI._getAccounts()[email.toLowerCase()];
      if (existing) return { ok: false, error: "An account with this email already exists" };
      const newUser = { uid: Math.random().toString(36).slice(2,10), name: name.trim(), email: email.trim().toLowerCase(), phone, location: location.trim(), sectors, createdAt: new Date().toISOString(), authProvider: "local" };
      AuthAPI._saveAccount(newUser, password);
  ...
  async signIn({ email, password }) {
    if (USE_FIREBASE) {
      ...
      try {
        ...
        const profile = JSON.parse(localStorage.getItem(`sl_profile_${cred.user.uid}`) || "null");
        const fbUser = profile || { uid: cred.user.uid, name: cred.user.displayName || email, email: email.toLowerCase(), phone: "", location: "", sectors: ["shop"], authProvider: "firebase" };
      ...
      // localStorage path
      const account = AuthAPI._getAccounts()[email.trim().toLowerCase()];
      if (!account) return { ok: false, error: "No account found with this email" };
      if (account._pw !== password) return { ok: false, error: "Incorrect password" };
  ...
  async resetPassword(email) {
    if (USE_FIREBASE) {
      ...
      const account = AuthAPI._getAccounts()[email.trim().toLowerCase()];
      if (!account) return { ok: false, error: "No account found with this email" };
      return { ok: true, message: "Password reset sent (demo — no email in local mode)" };
    }
  },

  // ── Internal localStorage helpers (not used when Firebase is active) ──
  _getAccounts() { try { return JSON.parse(localStorage.getItem("sl_accounts_v1")) || {}; } catch { return {}; } },
  _saveAccount(user, password) {
    const accounts = this._getAccounts();
    accounts[user.email] = { ...user, _pw: password };
    localStorage.setItem("sl_accounts_v1", JSON.stringify(accounts));
  },
};
