      ...
      <Section
        ...
        renderItem={item => (
          <>
            ...
            <div style={{ background: COLORS.danger, color: "#fff", borderRadius: 8, padding: "4px 10px", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>Restock</div>
          </>
        )}
      />
      ...
      <Section
        ...
        renderItem={item => (
          <>
            ...
            <div style={{ background: COLORS.amber, color: "#fff", borderRadius: 8, padding: "4px 10px", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{item.stock} left</div>
          </>
        )}
      />
    </div>
      ...
      <div className="app">
        ...
        <div className="main-wrap">
          <div className="topbar">
            ...
            <div className="topbar-right">
              ...
              {(() => {
                const debtKey = `sl_debt_${user?.uid}`;
                const recs = (() => { try { return JSON.parse(localStorage.getItem(debtKey)) || []; } catch { return []; } })();
                const invData = (() => { try { return JSON.parse(localStorage.getItem(`sl_inv_${user?.uid}`)) || []; } catch { return []; } })();
                const stockAlerts = invData.filter(i => i.stock === 0 || i.stock < 5).length;
                ...
                return (
                  <button onClick={() => setNavTab("notifications")} title="Notifications" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", color: urgentN > 0 ? COLORS.danger : COLORS.textMuted, padding: 6, borderRadius: 8, position: "relative", fontSize: 17 }}>
                    🔔
                    {urgentN > 0 && (
          ...
          <div className="main">
            ...
            {navTab === "notifications" && <NotificationsScreen user={user} />}
