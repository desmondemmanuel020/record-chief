...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { font-family: 'Sora', sans-serif; background: var(--bg); color: var(--text); transition: background 0.2s, color 0.2s; }
  ...
  .form-input { width: 100%; padding: 10px 12px; border-radius: 8px; border: 1px solid var(--border); background: var(--bg); font-size: 14px; color: var(--text); outline: none; font-family: 'Sora', sans-serif; transition: border-color 0.2s; }
  ...
  .btn { border: none; border-radius: 9px; padding: 11px 20px; font-size: 14px; font-weight: 600; font-family: 'Sora', sans-serif; transition: all 0.15s; display: inline-flex; align-items: center; gap: 6px; justify-content: center; }
  ...
  .search-bar { width: 100%; padding: 10px 36px 10px 38px; border-radius: 12px; border: 1.5px solid var(--border); background: var(--surface); font-size: 13px; font-family: 'Sora', sans-serif; outline: none; transition: border-color 0.2s, box-shadow 0.2s; color: var(--text); }
  ...
  .tab-btn { flex: 1; border: none; background: none; padding: 7px; font-size: 12px; font-weight: 500; border-radius: 8px; color: var(--text-muted); font-family: 'Sora', sans-serif; transition: all 0.15s; }
  ...
  .bottom-tab-item {
    ...
    cursor: pointer; font-family: 'Sora', sans-serif; font-size: 9px; font-weight: 600;
  ...
  .bottom-tab-item .btab-icon { font-size: 20px; line-height: 1; }
  ...
  .bottom-tab-bar { padding-bottom: env(safe-area-inset-bottom, 0px); height: calc(64px + env(safe-area-inset-bottom, 0px)); background: #0F172A; }
  ...
  @media (max-width: 640px) {
    ...
    .main { padding: 0.9rem; padding-bottom: calc(80px + env(safe-area-inset-bottom, 0px)); }
    ...
    .card-sm { padding: 0.65rem 0.8rem; }

    /* Full-width stat grid on very small screens */
    .stat-grid { grid-template-columns: 1fr 1fr; gap: 8px; }

    /* Larger touch inputs */
    .form-input { padding: 12px 14px; font-size: 16px; } /* 16px prevents iOS zoom */
    .search-bar { font-size: 16px; padding: 12px 36px 12px 40px; }
    ...
    /* Entry rows bigger touch area */
    .entry-row { padding: 14px 0; }
  ...
  return (
    <div className="welcome-screen" style={{ padding: 0, justifyContent: "stretch" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 2rem 2rem" }}>
        <div style={{ width: 80, height: 80, borderRadius: 24, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 42, marginBottom: 24, boxShadow: "0 8px 32px rgba(0,0,0,0.15)" }}>📒</div>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 26, fontWeight: 700, letterSpacing: "-0.5px", marginBottom: 8 }}>Record Chief</div>
        <div style={{ fontSize: 14, opacity: 0.75, textAlign: "center", maxWidth: 260, lineHeight: 1.6 }}>
      ...
      {/* Bottom action card */}
      <div style={{ background: "#fff", borderRadius: "28px 28px 0 0", padding: "28px 24px 36px", width: "100%" }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: COLORS.text, marginBottom: 6 }}>Get Started</div>
...
const getAllAccounts = () => { try { return JSON.parse(localStorage.getItem(ACCOUNTS_KEY)) || {}; } catch { return {}; } };
const saveAccount = (user, password) => {
  const accounts = getAllAccounts();
  accounts[user.email.toLowerCase()] = { ...user, _pw: password };
  localStorage.setItem(ACCOUNTS_KEY, JSON.stringify(accounts));
};
const findAccount = (email) => getAllAccounts()[email.toLowerCase()] || null;
...
function SignupScreen({ onAuth, onNavigate }) {
  ...
  const validate = () => {
    ...
    else if (findAccount(form.email)) e.email = "An account with this email already exists";
  ...
  const submit = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const newUser = { uid: uid(), name: form.name.trim(), email: form.email.trim().toLowerCase(), phone: form.phone, location: form.location.trim(), sectors: selectedSectors, createdAt: TS() };
      saveAccount(newUser, form.password);
      onAuth(newUser, selectedSectors);
    }, 900);
  };
...
function LoginScreen({ onAuth, onNavigate }) {
  ...
  const submit = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const account = findAccount(form.email.trim());
      if (!account) { setLoading(false); setError("No account found with this email. Please sign up."); return; }
      if (account._pw !== form.password) { setLoading(false); setError("Incorrect password. Please try again."); return; }
      const { _pw, ...user } = account;
      onAuth(user, user.sectors);
    }, 700);
  };

  const handleForgot = () => {
    if (!forgotEmail.trim() || !isValidEmail(forgotEmail)) { setForgotMsg("Enter a valid email address."); return; }
    const account = findAccount(forgotEmail.trim());
    if (!account) { setForgotMsg("No account found with that email."); return; }
    setForgotMsg(`Your password hint: your password has ${account._pw.length} characters. (Full reset coming soon.)`);
  };
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
        ...
        return (
          <div style={{ background: "#EFF6FF", border: "1.5px solid #BFDBFE", borderRadius: 12, padding: "10px 14px", marginBottom: "0.75rem", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={() => { setShowExport(true); localStorage.setItem(lastExportKey, new Date().toISOString()); }} style={{
              ...
              padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora', sans-serif",
            }}>Export Now</button>
          </div>
        );
      })()}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.6rem" }}>
        ...
        <button onClick={onManageSectors} style={{ background: "none", border: "none", color: COLORS.primary, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora', sans-serif", display: "flex", alignItems: "center", gap: 4 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
        <div>
          <div style={{ fontSize: 18, fontWeight: 700 }}>💼 Sales Tracker</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>{entries.length} entries total</div>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
      ...
      <div className="chip-row" style={{ marginBottom: "0.75rem", alignItems: "center" }}>
        <div style={{ fontSize: 12, color: COLORS.textMuted, fontWeight: 500, flexShrink: 0, marginRight: 2 }}>Sort:</div>
        {[
          ...
          <button key={opt.value} onClick={() => setSortBy(opt.value)} style={{
            ...
            border: sortBy === opt.value ? `1.5px solid ${COLORS.primary}` : `1px solid ${COLORS.border}`,
            background: sortBy === opt.value ? COLORS.primaryLight : COLORS.surface,
      ...
      {filtered.length === 0 ? (
        <div className="empty-state"><div className="empty-icon">📋</div><h3>No entries yet</h3><p>Tap the + button to add your first entry</p></div>
      ) : (
        <div className="card">
          {filtered.map(entry => {
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setShowFieldChoice(false)} style={{
              ...
              fontFamily: "'Sora', sans-serif", padding: "6px",
            }}>Cancel</button>
          </div>
        </div>
      )}

      {/* Entry bottom-sheet modal */}
      {showEntryForm && (
        <div style={{
          position: "fixed", inset: 0, zIndex: 300,
          ...
          <div style={{
            ...
            padding: "0 1rem 1.5rem", maxHeight: "92vh", overflowY: "auto",
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div style={{ fontSize: 16, fontWeight: 700 }}>{editId ? "Edit Entry" : "New Entry"}</div>
              <button onClick={() => { setShowEntryForm(false); setShowManageFields(false); setEditId(null); setForm({}); }} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.textMuted, fontSize: 20, lineHeight: 1, padding: 4 }}>×</button>
            </div>
            ...
            {!fields && !showManageFields && (
              <div style={{ background: COLORS.primaryLight, border: `1px solid ${COLORS.primary}30`, borderRadius: 12, padding: "12px 14px", marginBottom: 16, display: "flex", gap: 10, alignItems: "center" }}>
                ...
                <button onClick={() => setShowManageFields(true)} style={{ background: COLORS.primary, color: "#fff", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora', sans-serif", whiteSpace: "nowrap" }}>Add Fields</button>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              ...
              <button className="btn btn-primary" style={{ flex: 2 }} onClick={() => { saveEntry(); }}>{editId ? "Update Entry" : "Save Entry"}</button>
            </div>
          </div>
        </div>
      )}
...
function ShopScreen({ user }) {
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)}
                  style={{ width: "100%", padding: "7px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.15)", color: "#fff", fontSize: 12, fontFamily: "'Sora', sans-serif", outline: "none" }} />
              </div>
              <div style={{ flex: 1 }}>
              ...
              return allTags.length > 0 ? (
                <div className="chip-row" style={{ marginBottom: "0.65rem" }}>
                  <button onClick={() => setTagFilter("")} style={{
                    ...
                    fontFamily: "'Sora', sans-serif", border: tagFilter === "" ? `1.5px solid ${COLORS.primary}` : `1px solid ${COLORS.border}`,
                  ...
                  {allTags.map(tag => (
                    <button key={tag} onClick={() => setTagFilter(t => t === tag ? "" : tag)} style={{
                      padding: "4px 11px", borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: "pointer",
                      fontFamily: "'Sora', sans-serif", border: tagFilter === tag ? `1.5px solid ${COLORS.primary}` : `1px solid ${COLORS.border}`,
      ...
      {/* ── FAB ── */}
      <button
        onClick={() => { setForm({ itemId: "", qty: "" }); setErrors({}); setShowSaleForm(true); }}
        title="Record a sale"
        style={{
          position: "fixed", bottom: 28, right: 28, zIndex: 200,
          width: 56, height: 56, borderRadius: "50%",
      ...
      {/* ── Record Sale bottom-sheet modal ── */}
      {showSaleForm && (
        <div style={{
          position: "fixed", inset: 0, zIndex: 300,
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: COLORS.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🛍️</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>Record Sale</div>
              </div>
              <button onClick={() => setShowSaleForm(false)} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.textMuted, fontSize: 20, lineHeight: 1, padding: 4 }}>×</button>
            </div>
            {inventory.filter(i => i.stock > 0).length === 0 ? (
              <div style={{ textAlign: "center", padding: "1.5rem 0" }}>
                <div style={{ fontSize: 36, marginBottom: 8 }}>📦</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text, marginBottom: 4 }}>No items in stock</div>
                <div style={{ fontSize: 12, color: COLORS.textMuted }}>Switch to the Inventory tab to add items first.</div>
              </div>
            ) : (
              <>
                <div className="form-group">
                ...
                {selectedItem && form.qty && parseInt(form.qty) > 0 && !errors.qty && (
                  <div style={{ background: COLORS.accentLight, border: `1px solid #6EE7B7`, borderRadius: 12, padding: "12px 16px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: 11, color: COLORS.accent, fontWeight: 600 }}>TOTAL AMOUNT</div>
                      <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 1 }}>{form.qty} × {NAIRA(selectedItem.price)}</div>
                    </div>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 22, fontWeight: 700, color: COLORS.accent }}>{NAIRA(parseInt(form.qty || 0) * selectedItem.price)}</div>
                  </div>
                )}
                <div style={{ display: "flex", gap: 8 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "center", padding: "10px 0 4px" }}>
              <div style={{ width: 36, height: 4, borderRadius: 2, background: FG.border }} />
            </div>
            {/* sheet header */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: FG.pale, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🌾</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: FG.dark }}>Add Expenditure</div>
              </div>
              <button onClick={() => setShowForm(false)} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.textMuted, fontSize: 22, lineHeight: 1, padding: 4 }}>×</button>
            </div>
    ...
    <div>
      ...
      {showBackupReminder && (
        <div style={{ background: "#EFF6FF", border: "1.5px solid #BFDBFE", borderRadius: 12, padding: "10px 14px", marginBottom: "0.75rem", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => { setShowExport(true); dismissBackup(); }} style={{ background: COLORS.primary, border: "none", borderRadius: 8, padding: "6px 12px", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora', sans-serif", flexShrink: 0 }}>
            Export now
          </button>
...
function DebtCreditScreen({ user }) {
  ...
  const [editId, setEditId] = useState(null);
  const [toast, setToast] = useState(null);
  const [form, setForm] = useState({ type: "credit", name: "", amount: "", note: "", dueDate: "", date: TODAY() });
  const [errors, setErrors] = useState({});
  ...
  const save = () => {
    ...
    if (!form.amount || isNaN(form.amount) || parseFloat(form.amount) <= 0) e.amount = "Enter a valid amount";
    if (Object.keys(e).length) { setErrors(e); return; }
    const rec = { id: editId || uid(), type: form.type, name: form.name.trim(), amount: parseFloat(form.amount), note: form.note, dueDate: form.dueDate, date: form.date, settled: false, createdAt: editId ? records.find(r=>r.id===editId)?.createdAt : TS(), updatedAt: TS() };
    if (editId) {
    ...
    setForm({ type: "credit", name: "", amount: "", note: "", dueDate: "", date: TODAY() });
  ...
  const startEdit = (rec) => {
    setForm({ type: rec.type, name: rec.name, amount: String(rec.amount), note: rec.note || "", dueDate: rec.dueDate || "", date: rec.date || TODAY() });
    setEditId(rec.id);
    setTab("add");
  };
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        {!dimmed ? (() => {
            ...
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: `0.5px solid ${COLORS.border}` }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => settle(r.id)} style={{
                  ...
                  fontFamily: "'Sora', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                }}>
              ...
              {(() => {
                ...
                const [noteText, setNoteText] = React.useState(r.note || "");
                return (
                  <div style={{ marginTop: 6 }}>
                    <button onClick={() => setShowNote(v => !v)} style={{
                      width: "100%", padding: "7px 12px", border: `1px solid ${COLORS.border}`,
                      ...
                      {r.note && !showNote && <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontStyle: "italic", color: COLORS.textLight }}>{r.note}</span>}
                    </button>
                    {showNote && (
                      <div style={{ marginTop: 6 }}>
                        <textarea className="form-input" rows={2} placeholder="Write a note…"
                          value={noteText} onChange={e => setNoteText(e.target.value)}
                          style={{ resize: "none", fontSize: 13 }} />
                        <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                          <button onClick={() => { setNoteText(r.note || ""); setShowNote(false); }}
                            style={{ flex: 1, padding: "7px", border: `1px solid ${COLORS.border}`, borderRadius: 8, background: "transparent", color: COLORS.textMuted, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>Cancel</button>
                          <button onClick={() => {
                            setRecords(prev => prev.map(x => x.id === r.id ? { ...x, note: noteText.trim(), updatedAt: TS() } : x));
                            setShowNote(false);
                            showToast("Note saved!");
                          }} style={{ flex: 2, padding: "7px", border: "none", borderRadius: 8, background: accentColor, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>Save Note</button>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
            <button onClick={() => settle(r.id)} style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 7, padding: "5px 12px", fontSize: 11, color: COLORS.textMuted, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>
              Undo
            </button>
          </div>
        )}
      ...
      <div style={{ position: "relative", marginBottom: "0.75rem" }}>
        ...
        <input
          style={{
            ...
            fontSize: 13, fontFamily: "'Sora', sans-serif", outline: "none", color: COLORS.text,
              ...
              {bulkMode && bulkSelected.size > 0 && (
                <div style={{ marginBottom: 10, display: "flex", gap: 8 }}>
                  <button onClick={() => {
                    ...
                    setBulkMode(false);
                  }} style={{ flex: 1, padding: "10px", border: "none", borderRadius: 10, background: T.label, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>
                  ...
                  <button onClick={() => {
                    ...
                    setBulkSelected(new Set());
                    setBulkMode(false);
                  }} style={{ padding: "10px 14px", border: "none", borderRadius: 10, background: COLORS.dangerLight, color: COLORS.danger, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>
                    🗑️
                  </button>
                </div>
              )}
          ...
          {archivedRecords.map(r => (
            <div key={r.id} style={{ background: COLORS.bg, borderRadius: 12, padding: "10px 14px", marginBottom: 8, border: `1px solid ${COLORS.border}`, opacity: 0.7, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                <button onClick={() => restore(r.id)} style={{ background: COLORS.accentLight, border: "none", borderRadius: 7, padding: "5px 10px", fontSize: 11, color: COLORS.accent, cursor: "pointer", fontFamily: "'Sora', sans-serif", fontWeight: 600 }}>Restore</button>
                <button onClick={() => hardDelete(r.id)} style={{ background: COLORS.dangerLight, border: "none", borderRadius: 7, padding: "5px 10px", fontSize: 11, color: COLORS.danger, cursor: "pointer", fontFamily: "'Sora', sans-serif", fontWeight: 600 }}>Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {/* Debt option */}
              <button onClick={() => {
                setForm({ type: "debt", name: "", amount: "", note: "", dueDate: "", date: TODAY() });
                setShowTypeChoice(false);
                setTab("add");
              }} style={{
                padding: "14px 16px", borderRadius: 14, cursor: "pointer",
            ...
            <div className="form-group">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setForm(p=>({...p,type:"credit"}))} style={{
                  ...
                  cursor: "pointer", fontSize: 13, fontFamily: "'Sora', sans-serif", transition: "all 0.15s",
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: "0.75rem" }}>
        ...
        {(() => {
          ...
          const [shared, setShared] = React.useState(() => { try { return JSON.parse(localStorage.getItem(shareKey)) || []; } catch { return []; } });
                ...
                <div style={{ marginBottom: 10 }}>
                  {shared.map(em => (
                    <div key={em} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: `0.5px solid ${COLORS.border}` }}>
                      ...
                      <button onClick={() => removeShare(em)} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.danger, fontSize: 11, fontFamily: "'Sora', sans-serif" }}>Remove</button>
                    </div>
                  ))}
                </div>
              )}
          ...
          return (
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={() => { setPinSetup(v => !v); setP1(""); setP2(""); setPErr(""); }} style={{ background: currentPin ? COLORS.dangerLight : COLORS.primaryLight, border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Sora', sans-serif", color: currentPin ? COLORS.danger : COLORS.primary }}>
                  {currentPin ? "Remove PIN" : (pinSetup ? "Cancel" : "Set PIN")}
                </button>
              </div>
              {currentPin && !pinSetup && (
                <button onClick={() => { setCurrentPin(null); showToast && showToast("PIN removed"); }} style={{ marginTop: 8, width: "100%", padding: "8px", border: `1px solid ${COLORS.dangerLight}`, borderRadius: 8, background: COLORS.dangerLight, color: COLORS.danger, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>Remove PIN Lock</button>
              )}
              {pinSetup && !currentPin && (
      ...
      <div className="card">
        ...
        <button className="btn btn-outline" style={{ width: "100%", marginBottom: 8, justifyContent: "flex-start", gap: 10 }}
          onClick={() => setToast({ msg: "Password reset email sent! (demo)", type: "success" })}>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem", background: `linear-gradient(145deg, ${s.color}CC, ${s.color})`, color: "#fff", transition: "background 0.4s" }}>
      <div style={{ width: "100%", maxWidth: 360 }}>
        ...
        {step > 0 && (
          <button onClick={() => setStep(s => s-1)} style={{ width: "100%", marginTop: 10, padding: "10px", border: "1.5px solid rgba(255,255,255,0.4)", borderRadius: 14, background: "transparent", color: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "'Sora', sans-serif" }}>
            ← Back
          </button>
        )}
        <button onClick={onDone} style={{ width: "100%", marginTop: 8, background: "none", border: "none", color: "rgba(255,255,255,0.5)", fontSize: 13, cursor: "pointer", fontFamily: "'Sora', sans-serif", padding: "6px" }}>
          Skip
        </button>
      </div>
    </div>
...
function OfflineIndicator() {
  ...
  useEffect(() => {
    const goOnline  = () => { setOnline(true);  setShowBanner(true); timerRef.current = setTimeout(() => setShowBanner(false), 3000); };
    const goOffline = () => { setOnline(false); setShowBanner(true); };
    window.addEventListener("online",  goOnline);
    window.addEventListener("offline", goOffline);
    return () => { window.removeEventListener("online", goOnline); window.removeEventListener("offline", goOffline); clearTimeout(timerRef.current); };
  }, []);
  ...
  return (
    <div style={{
      ...
      {online ? "✅ Back online" : "📵 You're offline — changes save locally"}
    </div>
  );
}
...
function App() {
  ...
  const handleAuth = (u, sectors) => {
    ...
    setUser(fullUser);
    // default active sector to first selected
      ...
      <div className="app">
          ...
          <div className="topbar">
            ...
            <div className="topbar-right">
              ...
              {(() => {
                const debtKey = `sl_debt_${user?.uid}`;
                const recs = (() => { try { return JSON.parse(localStorage.getItem(debtKey)) || []; } catch { return []; } })();
                const urgentN = recs.filter(r => {
                  if (r.settled || !r.dueDate) return false;
                  const days = Math.ceil((new Date(r.dueDate) - new Date()) / 86400000);
                  return days <= 3;
                }).length;
                return (
                  <button onClick={() => setNavTab("debtcredit")} title="Debt reminders" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", color: urgentN > 0 ? COLORS.danger : COLORS.textMuted, padding: 6, borderRadius: 8, position: "relative", fontSize: 17 }}>
                    🔔
                    {urgentN > 0 && (
                      <span style={{ position: "absolute", top: 2, right: 2, width: 14, height: 14, borderRadius: "50%", background: COLORS.danger, color: "#fff", fontSize: 8, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{urgentN > 9 ? "9+" : urgentN}</span>
                    )}
                  </button>
                );
              })()}
            ...
            <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(15,23,42,0.6)", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: "4rem" }}
              ...
              <div style={{ width: "calc(100% - 32px)", maxWidth: 520, background: "#fff", borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", overflow: "hidden", animation: "scaleIn 0.18s ease" }}
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: `1px solid ${COLORS.border}`, gap: 10 }}>
                  ...
                  <input
                    autoFocus
                    style={{ flex: 1, border: "none", outline: "none", fontSize: 16, fontFamily: "'Sora', sans-serif", color: COLORS.text, background: "transparent" }}
                  ...
                  <button onClick={() => { setShowGlobalSearch(false); setGlobalSearch(""); }} style={{ background: COLORS.bg, border: "none", cursor: "pointer", color: COLORS.textMuted, fontSize: 11, fontWeight: 600, borderRadius: 6, padding: "4px 8px", fontFamily: "'Sora', sans-serif" }}>ESC</button>
                </div>
                {(() => {
        ...
        {(() => {
          ...
          const tabs = [
            ...
            ...(firstSector ? [{ id: "sector", icon: firstSector.icon, label: firstSector.id === "farm" ? "Farm" : firstSector.id === "sales" ? "Sales" : "Shop", accent: firstSector.id === "farm" ? "#40916C" : firstSector.id === "sales" ? "#A78BFA" : "#60A5FA" }] : []),
          ...
          return (
            <div className="bottom-tab-bar">
              {tabs.map(t => {
                ...
                return (
                  <button key={t.id} className={`bottom-tab-item${isActive ? " active" : ""}`}
                    onClick={() => {
                      if (t.id === "sector" && firstSector) setSector(firstSector.id);
                      setNavTab(t.id);
